...
CREATE TABLE `center_of_mass` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `UK__COM_AID_SID_ACTIVE_USER` (`prep_id`,`structure_id`,`active`,`person_id`,`input_type`),
...
) ENGINE=InnoDB AUTO_INCREMENT=64685 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `layer_data` (
  ...
  PRIMARY KEY (`id`),
  KEY `K__URLID` (`url_id`),
  CONSTRAINT `FK__URLID` FOREIGN KEY (`url_id`) REFERENCES `neuroglancer_urls` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=35195 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `~log` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=29895 DEFAULT CHARSET=utf8 COMMENT='event logging table for `active_atlas_development`';
/*!40101 SET character_set_client = @saved_cs_client */;
...
/*!50013 DEFINER=`dklab`@`%` SQL SECURITY DEFINER */
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dklab`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `task_view` AS select `A`.`prep_id` AS `prep_id`,count(distinct `PL`.`id`) / (select count(0) from `progress_lookup`) AS `percent_complete`,sum(case when `T`.`completed` = 1 then 1 else 0 end) AS `complete`,date_format(max(`T`.`created`),'%b %d, %Y') AS `created` from ((`animal` `A` left join `task` `T` on(`A`.`prep_id` = `T`.`prep_id`)) left join `progress_lookup` `PL` on(`T`.`lookup_id` = `PL`.`id`)) group by `A`.`prep_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-03-29 12:37:53
